              ...
              >
                  ...
                  <v-layout row wrap>
                    <v-flex justify-center align-center>
                      <h3 class="headline font-weight-bold text-xs-center primary--text">
                        FORM PENAMBAHAN JENIS TABUNGAN
                      </h3>
                    </v-flex>
                  </v-layout>
                </v-card-title>
                <v-card-text>
                  ...
                  <p class="black--text text-xs-justify">
                    Terima kasih karena telah melakukan pengajuan pengambahan jenis tabungan di bank 
                    <strong>bjb</strong>. 
                  ...
                  <v-layout column wrap justify-center align-center>
                    <v-flex>
                      <v-btn 
                        href="/formPerubahanDataTabungan"
                        color="primary"
                      >
                        Form Perubahan Data
                      </v-btn>
...
export default {
  ...
  methods: {
    getSavingTypes(nik) {
      api.getAvailableSavingTypes(nik).then(response => {
        this.savingTypes = response.data;
      });
    },
    validateForm1(e) {
      e.preventDefault();
        ...
        this.$http
          ...
          .then(
            response => {
              ...
              // Remove Existing Saving Type
              // for (let i = 0; i < this.savingTypes.length; i++) {
              //   let savingType = this.savingTypes[i];
              //   if (savingType.code == this.item.type) {
              //     this.savingTypes.splice(i, 1);
              //   }
              // }
              this.getSavingTypes(this.item.nik);
    ...
    validateForm3() {
      if (this.$refs.form3.validate()) {
        this.loadingDialog = true;
        this.item.additional = true;
        delete this.item.ticket_number;
        // Get Ticket number from backend
        api
          .getTicketNumber()
          .then(response => {
            // Insert new ticket Number
            this.item.ticket_number = response.data.ticket_number;

            this.$http
              .post(
                "http://103.217.173.123:5020/api/v1/account_registration",
                this.item,
                {
                  headers: {
                    Authorization:
    ...
    downloadPdf() {
      window.open(
        "http://103.217.173.123:5020/api/v1/download_pdf/" +
          this.responseSuccess.ticket_number +
          ".pdf",
        "_blank"
      );
    }
